    ...
    var body: some View {
        NavigationStack{
            GeometryReader { reader in
                ...
                .task {
                    userModel.load()
                    stage = StageModel(prompt: ["oke", "ok"], listImage: [], resultParent: [], resultChild: [])
...
struct Stage1View_Previews: PreviewProvider {
    static var previews: some View {
        ...
        Stage1View(userModel: UserViewModel(), stage: StageModel(prompt: [""], listImage: emptyImageModel, resultParent: emptyImageModel, resultChild: emptyImageModel), stageViewModel: StageViewModel())
